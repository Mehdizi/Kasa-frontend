...
export const AccordeonItem = ({ title, contentComponent }: { title: string, contentComponent: ReactNode }) => {

  const [isArrowDeployed, setIsArrowDeployed] = useState(false)
  ...
  return (
    <article className='accordeon-container'>
      <div className='wrapper-title'>
        <h2>{title}</h2>
        <button onClick={() => toggleAccordeon()}>
          <svg className={!isArrowDeployed ? "arrow-up" : "arrow-down"} width="24" height="14" viewBox="0 0 24 14" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className={!isArrowDeployed ? "wrapper-description" : "wrapper-description appear"}>
        {contentComponent}
