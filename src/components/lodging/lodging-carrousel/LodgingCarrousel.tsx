...
export const LodgingCarrousel = ({ pictures }: { pictures: string[] }) => {
  ...
  return (
    <div className="lodging-carrousel-wrapper">
      <img
        className="lodging-carrousel-img"
        src={pictures[actualpicture]}
        alt="House photography"
      />
      <button
        className="previous-button carrousel-button"
        onClick={goToPreviousPicture}>
        {arrowLeft}
      </button>
      <button
        className="next-button carrousel-button"
        onClick={goToNextPicture}>
        {arrowRight}
      </button>
      <p className="current-img">
        {actualpicture + 1}/{pictureNumber}
      </p>
    </div>
  );
};
